...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def algo3_defense_round_u(self, game_state):
        wall_pos = [[ 0, 13],[ 1, 13],[ 2, 13],[ 25, 13],[ 26, 13],[ 27, 13],[ 3, 12],[ 24, 12],[ 4, 11],[ 23, 11],[ 5, 10],[ 6, 10],[ 21, 10],[ 22, 10],[ 7, 9],[ 8, 9],[ 9, 9],[ 10, 9],[ 11, 9],[ 12, 9],[ 15, 9],[ 16, 9],[ 17, 9],[ 18, 9],[ 19, 9],[ 20, 9]]
        turret_pos = [[ 1, 12],[ 2, 12],[ 25, 12],[ 26, 12],[ 3, 11],[ 24, 11],[ 9, 8],[ 10, 8],[ 11, 8],[ 12, 8],[ 15, 8],[ 16, 8],[ 17, 8],[ 18, 8]]
        wall_pos_critical = [[ 0, 13],[ 1, 13],[ 2, 13],[ 25, 13],[ 26, 13],[ 27, 13]]
        deploy_pos = [[ 13, 0]]
        game_state.attempt_spawn(WALL, wall_pos)
        game_state.attempt_spawn(TURRET, turret_pos)
        game_state.attempt_upgrade(wall_pos_critical)
        game_state.attempt_upgrade(turret_pos)

        game_state.attempt_spawn(INTERCEPTOR, deploy_pos, 2)
        ...
        if game_state.turn_number == 0:
            ...
            if game_state.get_resource(MP) < 18:
                ...
                if self.detect_enemy_unit(game_state, None, valid_x = {i for i in range(0, 28)}, valid_y = [14, 15, 16]) >= 30 and game_state.get_resource(SP) >= 54:
                    ...
                    if self.algo3_counter % 4 == 0 and self.detect_enemy_unit(game_state, WALL, valid_x = {i for i in range(0, 28)}, valid_y = {i for i in range(14, 28)}) >= 20:
                        self.algo3_attack_round_demo(game_state)
                    elif self.algo3_counter % 4 == 3 and game_state.get_resource(SP) >= 50:
                        self.algo3_defense_round_u(game_state)
                    else:
