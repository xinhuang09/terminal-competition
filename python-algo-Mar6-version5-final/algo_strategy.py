...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.algo3_v_turret = [[ 1, 12],[ 24, 12], [ 2, 12],[ 3, 12],[ 25, 12],[ 26, 12],[ 6, 11],[ 21, 11]]
        self.algo3_v_turret2 = [[5, 11], [22, 11], [ 6, 12],[ 21, 12],[ 3, 11],[ 24, 11],[ 5, 10],[ 22, 10]]
        self.algo3_v_turret3 = [[ 3, 11],[ 7, 11],[ 20, 11],[ 24, 11],[ 5, 10],[ 22, 10], [3, 10], [24, 10]]
    ...
    def algo3_round1(self, game_state):
        ...
        """
        turret_pos = [[ 12, 13]]
        turret_pos2 = [[17, 13], [22, 13], [4, 13]]
        support_pos = [[ 11, 12],[ 10, 11],[ 9, 10],[ 8, 9]]
        support_upgrade_pos = [[ 11, 12],[ 10, 11]]
        deploy_pos = [[ 7, 6]]
        deploy_pos1 = [[3, 10]]
        ...
        game_state.attempt_spawn(TURRET, turret_pos2)

        game_state.attempt_spawn(INTERCEPTOR, deploy_pos1)
        game_state.attempt_spawn(SCOUT, deploy_pos, 1000)

        game_state.attempt_remove(support_pos)
        game_state.attempt_remove(turret_pos)
        game_state.attempt_remove(turret_pos2)

    def algo3_defense_round(self, game_state):
        moving_wall_pos = [[4, 10], [23, 10]]
        wal_p_index = random.randint(0, len(moving_wall_pos) - 1)
        wal_p = moving_wall_pos[wal_p_index]
        deploy_pos1 = [[4, 9]]
        deploy_pos2 = [[23, 9]]
        game_state.attempt_spawn(WALL, self.algo3_v_wall)
        if game_state.turn_number >= 3 and game_state.get_resource(MP, player_index = 1) >= 16:
            game_state.attempt_spawn(WALL, wal_p)
            if wal_p[0] <= 15:
                game_state.attempt_spawn(INTERCEPTOR, deploy_pos2)
            else:
                game_state.attempt_spawn(INTERCEPTOR, deploy_pos1)
        ...
        game_state.attempt_remove(wal_p)
    ...
    def algo3_attack_round(self, game_state):
        ...
        if self.detect_enemy_unit(game_state, WALL, valid_x = {i for i in range(0, 28)}, valid_y = {i for i in range(15, 22)}) >= 24:
            game_state.attempt_spawn(DEMOLISHER, deploy_pos, 1000)
        else:
            game_state.attempt_spawn(SCOUT, deploy_pos, 1000)
